...
int[] explosionsList = {};
float[] powerUpsList = {};
int nebulaRandomizer, backGroundRandomizer, nebulaPosRandomizerX, 
  nebulaPosRandomizerY;
PFont font;
...
void drawShip() {
  ...
  if (crashCounter < 90) {
    ...
    image(ship, 0, 0); 
    if ( crashCounter != 0 && (frameCount % 6 == 0 || frameCount % 5 == 0 
      || frameCount % 4 == 0) ) {
      image(bubble, 0, 0);
    }
...
void drawBackGround() {
  ...
  tint(75);
  image(nebulaImages[nebulaRandomizer], nebulaPosRandomizerX, 
    nebulaPosRandomizerY++);
  noTint();
}
...
void drawEffects() {
  ...
  for (int i = 0; i <= explosionsList.length-1; i=i+3) {
    if (explosionsList[i]<17) {
      image(explosionImages[explosionsList[i]], explosionsList[i+1], 
        explosionsList[i+2]);
      if (frameCount % 2 == 0) {
  ...
  for (int i = 0; i <= powerUpsList.length-1; i=i+5) {
    ...
    if (powerUpsList.length > 2) {
      if (dist(shipLoc.x, shipLoc.y, powerUpsList[i+1], powerUpsList[i+2])
        < bigRockSize/2 + ship.width/2) { 
        if (powerUpsList[i] == 1) { 
          lives++;
        } else if (powerUpsList[i] == 2) { 
          powerUpBubble = 400;
...
void collisionDetection() {
  if (crashCounter == 0) { 
    for (int i = 0; i < astroNums; i ++) {
      if (dist(shipLoc.x, shipLoc.y, astroids[i].x, astroids[i].y) < 
        bigRockSize/2 + ship.width/2 +(swatter+61) && !hit[i]) {
        hit[i] = true;
        if ( swatter > -60 ) {
          score++;
          crashCounter = 15;
      ...
      } else if (dist(shipLoc.x, shipLoc.y, sAstroOne[i].x, sAstroOne[i].y)
        < smallRockSize/2 + ship.width/2 +(swatter+61) && !destroyedOne[i]) {
        destroyedOne[i] = true;
        if ( swatter > -60 ) {
          score++;
          crashCounter = 7;
      ...
      } else if (dist(shipLoc.x, shipLoc.y, sAstroTwo[i].x, sAstroTwo[i].y)
        < smallRockSize/2 + ship.width/2 +(swatter+61) && !destroyedTwo[i]) {
        destroyedTwo[i] = true;
        if ( swatter > -60 ) {
          score++;
          crashCounter = 7;
      ...
      } else if (dist(shipLoc.x, shipLoc.y, sAstroThree[i].x, sAstroThree[i].y)
        < smallRockSize/2 + ship.width/2 +(swatter+61) && !destroyedThree[i]) {
        destroyedThree[i] = true;
        if ( swatter > -60 ) {
          score++;
          crashCounter = 7;
...
void startScreen () {
  ...
  text("controls: w/W/UP - thrust, a/A/LEFT - rotate left, d/D/RIGHT - rotate right,\n"
    + "SPACEBAR - fire, s - power up, p - pause, o - resume", width/2, 
    .8*height+20);
  popMatrix();
  if (mouseX > width/2 - 210 && mouseX < width/2 + 210 && mouseY > .4*height-50
    && mouseY < .4*height+50) {
    startButton = true;
  ...
  if (mouseX > width/2 - 210 && mouseX < width/2 + 210 && mouseY > .6*height-50
    && mouseY < .6*height+50) {
    exitButton = true;
    if (mousePressed && restartButton == false) {
      exit();
...
void gameOverScreen (String gameState) {
  ...
  popMatrix();
  if (mouseX > width/2 - 210 && mouseX < width/2 + 210 && mouseY > .7*height-50
    && mouseY < .7*height+50) {
    if (mousePressed) {
      resetGame();
      startScreen = true;
...
boolean shotCollision(float astroidX, float astroidY, int rockSize, 
  ...
  rockSize/=2;
  for (int i = 0; i < shots.size(); i++) {
    if ((shots.get(i).x >= (astroidX - rockSize)) && (shots.get(i).x 
      <= (astroidX + rockSize)) 
      && (shots.get(i).y >= (astroidY - rockSize)) && (shots.get(i).y 
      <= (astroidY + rockSize))) {
      if (!dead) {
        collision = true;
        if (!powerShot) {
...
void createAstro() {
  for (int i = 0; i < astroNums; i++) {
    astroids[i] = new PVector(0, random(0, height));
    astroDirect[i] = new PVector(random(-astroSpeed, astroSpeed), 
      random(-astroSpeed, astroSpeed));
    ...
    sAstroDirectOne[i] = new PVector(random(-sAstroSpeed, sAstroSpeed), 
      random(-sAstroSpeed, sAstroSpeed));
    sAstroDirectTwo[i] = new PVector(random(-sAstroSpeed, sAstroSpeed), 
      random(-sAstroSpeed, sAstroSpeed));
    sAstroDirectThree[i] = new PVector(random(-sAstroSpeed, sAstroSpeed), 
      random(-sAstroSpeed, sAstroSpeed));
    hit[i] = false;
